...
VALUES 
('bbq01', 'Premium Cup Otah (Raw)', 'Fresh mixed fish, coconut milk, chilli paste, in house secret recipe chilli paste spices, salt & sugar.','13.00', '5000'),
('bbq02', 'Chicken Satay (Raw)', 'Chicken thigh meat, cumin seeds, turmeric, coriander seeds, onion, garlic, brown sugar, salt & herbs.','12.50', '5000'),
('bbq03', 'Buffalo Chicken Wing (Smoke)', 'Mid joint chicken wing, homemade smoke BBQ sauce, garlic and sesame oil.','14.40', '5000'),
...
CREATE TABLE `orderdetails` (
    ...
    constraint bbqorderdetails_fk2 foreign key(itemid1) references bbqorder.inventory(itemid),
    constraint bbqorderdetails_fk3 foreign key(itemid2) references bbqorder.inventory(itemid),
    constraint bbqorderdetails_fk4 foreign key(itemid3) references bbqorder.inventory(itemid),
    constraint bbqorderdetails_fk5 foreign key(itemid4) references bbqorder.inventory(itemid),
    constraint bbqorderdetails_fk6 foreign key(itemid5) references bbqorder.inventory(itemid)
);
